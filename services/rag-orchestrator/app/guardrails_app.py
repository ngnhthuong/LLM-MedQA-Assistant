...
class ExternalInferenceLLM(BaseLLM):
    """
    NeMo Guardrails custom LLM provider
    backed by the existing external inference service (KServe).
    """

    def generate(self, prompt: str | None = None, messages: list | None = None, **kwargs) -> str:
...
def generate_with_guardrails(user_message: str, grounded_prompt: str) -> str:
    ...
    # Normalize Guardrails output safely
